...
%% tuning at single pixel level - (old): peak in a short window [UNA TANTUM: already done, now can skip]
...
%% tuning at single pixel level - averaging in a bigger window to reduce noise [UNA TANTUM: already done, now can skip]
% i_resp = 4:16;
% [xvar, ia, ic] = unique([chunks.pips.carrierFreqLevels; chunks.tones.carrierFreqLevels(1)]);
% useChunks = [3,5];
% for i = 1:2
...
keepPixels_AMMCipsi = false(size(pixel2keep));
keepPixels_AMMCipsi(:,keepZetas_AMMCipsi) = true;
keepPixels_AMMCipsi = keepPixels_AMMCipsi(:);
keepPixels_AMMCipsi = keepPixels_AMMCipsi(pixel2keep(:)); %OK

% manually select ipsi WED planes (only include planes where ipsi tonotopy can be separated) - incl tdT+
keepZetas_TONOTipsi = [19    20    21    24    25    26    16     5     6   11    12    14    15    36   39    40    41 ]; %updated on 12/12
keepPixels_TONOTipsi = false(size(pixel2keep));
keepPixels_TONOTipsi(:,keepZetas_TONOTipsi) = true;
keepPixels_TONOTipsi = keepPixels_TONOTipsi(:);
keepPixels_TONOTipsi = keepPixels_TONOTipsi(pixel2keep(:)); %OK
...
keepPixels_TONOTcontra = false(size(pixel2keep));
keepPixels_TONOTcontra(:,keepZetas_TONOTcontra) = true;
keepPixels_TONOTcontra = keepPixels_TONOTcontra(:);
keepPixels_TONOTcontra = keepPixels_TONOTcontra(pixel2keep(:)); %OK


keepZetas_TONOTboth = unique(cat(2, keepZetas_TONOTipsi, keepZetas_TONOTcontra)) ;
keepPixels_TONOTboth = false(size(pixel2keep));
keepPixels_TONOTboth(:,keepZetas_TONOTboth) = true;
keepPixels_TONOTboth = keepPixels_TONOTboth(:);
keepPixels_TONOTboth = keepPixels_TONOTboth(pixel2keep(:)); %OK


%% [UNA TANTUM: already done, now can skip]
...
load('/Users/galileo/Dropbox (HMS)/Data/WEDAMMC_piezo/downsampled/rawDataLinkage/ward_ZSCpix/pixelwisePeakFT_mean.mat') %171204
sizeMap = [60, 86];
load('klust_maxClust19.mat'), nK = length(unique(Tparent)); 
...
% double check the cluster-averaged tuning curves
for k = 1 : 19
    clustAvgTC(k,:) = nanmean(dff_peak(Tparent==k,:));
end

[hfig, hax] = figureTracesI_PP_pixels( 19, 160, 200);
for k = 1:19
    axes(hax(k,1)); hold on
    plot(xvar, clustAvgTC(k,:));
    ylabel(k)
    hax(k,1).FontSize = 4;
    hax(k,1).XAxis.Scale = 'log';
    axis tight
end

% this works as it should so far
...
%% AMMCipsi - something is wrong in this block
AMMCipsi_Ki = bsxfun(@and, ROIIndices, (pxAMMC & keepPixels_AMMCipsi) ); %okPP#171205
% % remove the excluded clusters (not freq-responsive) % DO NOT JUST YET
% AMMCipsi_Ki(:,CNS.excludeKs) = [];

flyNumUnique = unique(dataTable.fly(keepZetas_AMMCipsi));
tcAMMCipsi_clusterTuningFly = nan( size(ROIIndices,2), size(dff_peak,2) , length(flyNumUnique) );
tcAMMCipsi_sumPixels_clusterFly = nan(size(ROIIndices,2),length(flyNumUnique) );
for f = 1 : length(flyNumUnique)
    
    zetas = intersect( find(dataTable.fly == flyNumUnique(f)), keepZetas_AMMCipsi);
    builder = false(size(pixel2keep));
    builder(:,zetas) = true;
    idx_thisFly = builder(pixel2keep(:)); %all significant pixels belonging to this fly
    
    indices_K_thisFly = bsxfun(@and, AMMCipsi_Ki, idx_thisFly);        
    
    tcAMMCipsi_sumPixels_clusterFly(:,f) = sum(indices_K_thisFly);
    % say we do not consider clusters where there's only a fwe pixels per
    % fly 
    for k = 1:19
        tcAMMCipsi_clusterTuningFly(k,:,f) = nanmean(dff_peak( indices_K_thisFly(:, k) ,:), 1); % OK NOW
    end
end


% % double check the cluster-averaged tuning curves % THIS DOES WORK  %okPP#171205
% for k = 1 : 19
%     clustAvgTC_AMMC(k,:) = nanmean(dff_peak(AMMCipsi_Ki(:,k),:));
% end
% [hfig, hax] = figureTracesI_PP_pixels( 19, 160, 200);
% for k = 1:19
%     axes(hax(k,1)); hold on
%     plot(xvar, clustAvgTC_AMMC(k,:));
%     ylabel([k, sum(AMMCipsi_Ki(:,k)) ])
%     hax(k,1).FontSize = 4;
% end 
% 
% % %% redo ex novo (AMMCi) cluster fly separation
% % AMMCipsi_Ki = bsxfun(@and, ROIIndices, (pxAMMC & keepPixels_AMMCipsi) ); %okPP#171205                        % 49608          19
% % 
% % % just fly 182
% % flyNumUnique = 182;
% % 
% % zetas = intersect( find(dataTable.fly == flyNumUnique), keepZetas_AMMCipsi);
% % builder = false(size(pixel2keep));
% % builder(:,zetas) = true;
% % idx_thisFly = builder(pixel2keep(:));   % idx of all significant pixels belonging to this fly                 % 49608           1
% % 
% % % indices_K_thisFly = AMMCipsi_Ki(idx_thisFly, :);  % wrong!!!
% % indices_K_thisFly = bsxfun(@and, AMMCipsi_Ki, idx_thisFly);                                                   % 49608          19 % OKK
% % 



%% TONOTipsi
TONOTipsi_Ki = bsxfun(@and, ROIIndices, (pxITONOT & keepPixels_TONOTipsi) );

flyNumUnique = unique(dataTable.fly(keepZetas_TONOTipsi));
tcTONOTipsi_clusterTuningFly = nan( size(ROIIndices,2), size(dff_peak,2) , length(flyNumUnique) );
tcTONOTipsi_sumPixels_clusterFly = nan(size(ROIIndices,2),length(flyNumUnique) );
for f = 1 : length(flyNumUnique)
    
    zetas = intersect( find(dataTable.fly == flyNumUnique(f)), keepZetas_TONOTipsi);
    builder = false(size(pixel2keep));
    builder(:,zetas) = true;
    idx_thisFly = builder(pixel2keep(:)); %all significant pixels belonging to this fly
    
    indices_K_thisFly = bsxfun(@and, TONOTipsi_Ki, idx_thisFly);        
    
    tcTONOTipsi_sumPixels_clusterFly(:,f) = sum(indices_K_thisFly);
    % say we do not consider clusters where there's only a fwe pixels per
    % fly 
    for k = 1:19
        tcTONOTipsi_clusterTuningFly(k,:,f) = nanmean(dff_peak( indices_K_thisFly(:, k) ,:), 1); % OK NOW
    end
end
...
%% TONOTcontra
TONOTcontra_Ki = bsxfun(@and, ROIIndices, (pxWED & keepPixels_TONOTcontra) );

flyNumUnique = unique(dataTable.fly(keepZetas_TONOTcontra));
tcTONOTcontra_clusterTuningFly = nan( size(ROIIndices,2), size(dff_peak,2) , length(flyNumUnique) );
tcTONOTcontra_sumPixels_clusterFly = nan(size(ROIIndices,2),length(flyNumUnique) );
for f = 1 : length(flyNumUnique)
    
    zetas = intersect( find(dataTable.fly == flyNumUnique(f)), keepZetas_TONOTcontra);
    builder = false(size(pixel2keep));
    builder(:,zetas) = true;
    idx_thisFly = builder(pixel2keep(:)); %all significant pixels belonging to this fly
    
    indices_K_thisFly = bsxfun(@and, TONOTcontra_Ki, idx_thisFly);        
    
    tcTONOTcontra_sumPixels_clusterFly(:,f) = sum(indices_K_thisFly);
    % say we do not consider clusters where there's only a fwe pixels per
    % fly 
    for k = 1:19
        tcTONOTcontra_clusterTuningFly(k,:,f) = nanmean(dff_peak( indices_K_thisFly(:, k) ,:), 1); % OK NOW
    end
end


%% TONOTboth
% TONOTboth_Ki = bsxfun(@and, ROIIndices, (pxITONOT & keepPixels_TONOTboth) );
TONOTboth_Ki = TONOTcontra_Ki | TONOTipsi_Ki;


% % remove the excluded clusters (not freq-responsive)
% TONOTcontra_Ki(:,CNS.excludeKs) = [];

flyNumUnique = unique(dataTable.fly(keepZetas_TONOTboth));
tcTONOTboth_clusterTuningFly = nan( size(ROIIndices,2), size(dff_peak,2) , length(flyNumUnique) );
tcTONOTboth_sumPixels_clusterFly = nan(size(ROIIndices,2),length(flyNumUnique) );
for f = 1 : length(flyNumUnique)
    
    zetas = intersect( find(dataTable.fly == flyNumUnique(f)), keepZetas_TONOTboth);
    builder = false(size(pixel2keep));
    builder(:,zetas) = true;
    idx_thisFly = builder(pixel2keep(:)); %all significant pixels belonging to this fly
    
    indices_K_thisFly = bsxfun(@and, TONOTboth_Ki, idx_thisFly);        
    
    tcTONOTboth_sumPixels_clusterFly(:,f) = sum(indices_K_thisFly);
    % say we do not consider clusters where there's only a fwe pixels per
    % fly 
    for k = 1:19
        tcTONOTboth_clusterTuningFly(k,:,f) = nanmean(dff_peak( indices_K_thisFly(:, k) ,:), 1); % OK NOW
    end
end
...
[hfig, hax] = figureTracesI_PP_pixels( 19, 160*ones(1,4), 200 );
for k = 1:19
    axes(hax(k,1)); hold on
    for f = 1:size(tcAMMCipsi_clusterTuningFly,3)
        plot(xvar, squeeze(tcAMMCipsi_clusterTuningFly(k,:,f)), 'Color', colors(f,:));
    end
    ylabel(k)
    hax(k,1).FontSize = 4;
end
...
for k = 1:19
    ...
    for f = 1:size(tcTONOTipsi_clusterTuningFly,3)
        plot(xvar, squeeze(tcTONOTipsi_clusterTuningFly(k,:,f)), 'Color', colors(f,:));
    end
    hax(k,2).FontSize = 4;
end

for k = 1:19
    axes(hax(k,3)); hold on
    for f = 1:size(tcTONOTcontra_clusterTuningFly,3)
        plot(xvar, squeeze(tcTONOTcontra_clusterTuningFly(k,:,f)), 'Color', colors(f,:));
    end
    hax(k,3).FontSize = 4;
end

for k = 1:19
    axes(hax(k,4)); hold on
    for f = 1:size(tcTONOTboth_clusterTuningFly,3)
        plot(xvar, squeeze(tcTONOTboth_clusterTuningFly(k,:,f)), 'Color', colors(f,:));
    end
    hax(k,4).FontSize = 4;
end
...
axes(hax(1,1)); title('AMMCipsi, mean')
axes(hax(1,2)); title('WEDTONipsi')
axes(hax(1,3)); title('WEDTONcontra')
axes(hax(1,4)); title('WEDTONboth')

% % export_fig('ftuningCurves_dffMEAN_flyCluster_AMMCi_TONOTi_TONOTc.eps')
% export_fig('ftuningCurves_dffPEAK_flyCluster_AMMCi_TONOTi_TONOTc.eps')

% HOW CAN THEY POSSIBLY SUCK THIS MUCH?


testcheck = nanmean(cat(3, tcAMMCipsi_clusterTuningFly, tcTONOTboth_clusterTuningFly), 3);
[hfig, hax] = figureTracesI_PP_pixels( 19, 160, 200);
for k = 1:19
    axes(hax(k,1)); hold on
    plot(xvar, testcheck(k,:), 'Color', colors(f,:));
    ylabel(k)
    hax(k,1).FontSize = 4;
end
...
yvar2WED = interp1(xvar, dff_peak(pxITONOT,:)', x2 );
