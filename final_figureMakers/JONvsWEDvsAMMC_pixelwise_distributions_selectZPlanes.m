...
tc.AMMC = interp1(xvar, CNS.dff_peak(CNS.includedK_i & CNS.pxAMMC & keepPixels_AMMCipsi,:)', x2 );
...
tc.TONOTipsi = interp1(xvar, CNS.dff_peak(CNS.includedK_i & CNS.pxWED & keepPixels_TONOTipsi,:)', x2 );
cdf.TONOTipsi = bsxfun(@rdivide, cumsum(tc.TONOTipsi), max(cumsum(tc.TONOTipsi)) );

%% contra TONOTOPY freq-responsive pixels cdfs
tc.TONOTcontra = interp1(xvar, CNS.dff_peak(CNS.includedK_i & CNS.pxWED & keepPixels_TONOTcontra,:)', x2 );
