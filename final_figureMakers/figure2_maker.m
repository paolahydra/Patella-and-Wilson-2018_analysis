...
folder2stimuli = '/Users/galileo/Dropbox (HMS)/Paola_Manuscript/figures/figure2';
% folder2figure1 = '/Users/galileo/Dropbox (HMS)/figures/figure1';
folder2figure2 = '/Users/galileo/Dropbox (HMS)/Paola_Manuscript/figures/figure2';
...
for Ki = 1:nK
    ...
    avg = mean(allR,1);
    plot([ts_dec_all(1); ts_dec_all; ts_dec_all(end); ts_dec_all(1)], [0, avg, 0, 0], 'Color', colors(Ki,:)), hold on, axis tight
...
export_fig(fullfile(folder2figure2, 'chirps_spectrogram.eps'))
colorbar off
ax.Box = 'on';
ax.XTick = [];
ax.YTick = [];
...
export_fig(fullfile(folder2figure2, 'chirps_spectrogram.jpg'), '-m4')
...
for Ki = 1:nK
    ...
    avg = mean(allR,1);
    plot([ts_dec_all_sel(1); ts_dec_all_sel; ts_dec_all_sel(end); ts_dec_all_sel(1)], [0, avg(i_Tr), 0, 0], 'Color', colors(Ki,:)), hold on, axis tight
...
export_fig(fullfile(folder2figure2, 'tone_spectrogram.eps'))
colorbar off
ax.Box = 'on';
ax.XTick = [];
ax.YTick = [];
...
ax.XAxis.Color = [1,1,1];
ax.YAxis.Color = [1,1,1];
export_fig(fullfile(folder2figure2, 'tone_spectrogram.jpg'), '-m4')
...
for i = 1:2
    ...
    for Ki = 1:nK
        ...
        avg = mean(allR,1);
        plot([ts_dec_all_sel(1); ts_dec_all_sel; ts_dec_all_sel(end); ts_dec_all_sel(1)], [0, avg, 0, 0], 'Color', colors(Ki,:)), hold on, axis tight
        % plot([ts_dec_all_sel(1), ts_dec_all_sel(end)], [0,0], 'Color', colors(Ki,:) )
    ...
    colormap(cmap2)
    ax.XLim =  [ts_Stim_all_sel(1), ts_Stim_all_sel(end)];
    export_fig(fullfile(folder2figure2, sprintf('songs_%s_spectrogram.eps',num2str(sel_stN))))
    colorbar off
    ax.Box = 'on';
    ax.XTick = [];
    ax.YTick = [];
    ...
    ax.XAxis.Color = [1,1,1];
    ax.YAxis.Color = [1,1,1];
    export_fig(fullfile(folder2figure2, sprintf('songs_%s_spectrogram.jpg',num2str(sel_stN))), '-m4')
...
colormap(cmap2)
ax.XLim =  [ts_Stim_all(1), ts_Stim_all(end)];
export_fig(fullfile(folder2figure2, 'steps_spectrogram.eps'))
colorbar off
ax.Box = 'on';
ax.XTick = [];
ax.YTick = [];
...
ax.XAxis.Color = [1,1,1];
ax.YAxis.Color = [1,1,1];
export_fig(fullfile(folder2figure2, 'steps_spectrogram.jpg'), '-m4')
...
%     yvar = squeeze(chunks.(CN).amplitude_1_ft.zsBasExt(Ki,:,:));
%     plot(xvar, nanmean(yvar,2),'Color', colors(Ki,:)*0.3), hold on
%     
%     xvar = chunks.(CN).carrierFreqLevels;
%     yvar = squeeze(chunks.(CN).amplitude_2_ft.zsBasExt(Ki,:,:));
%     plot(xvar, nanmean(yvar,2),'Color', colors(Ki,:)*0.65), hold on
%     
%     xvar = chunks.(CN).carrierFreqLevels;
%     yvar = squeeze(chunks.(CN).amplitude_3_ft.zsBasExt(Ki,:,:));
%     plot(xvar, nanmean(yvar,2),'Color', colors(Ki,:)), hold on
%     
...
% export_fig(fullfile(folder2figure2, 'resp_pips_ft.eps'))
...
windowsize = 5; %for moving average
...
windowsize = 1; %for moving average
fTC = freqTuning_pipsTones_extendedBaselines(trialTypes, ROIs, windowsize, saveNameFTData); % baseline+ext baseline: 102 points
% load('fTC_data.mat') %beacuse previous two lines have been already aveluated.
...
%% combine 4Hz plus pips, and plot
[xvar, ia, ic] = unique([chunks.pips.carrierFreqLevels; chunks.tones.carrierFreqLevels]);
...
for Ki = 1:11   %fix
    ...
    yvar1 = yvar1(ia);
    yvar1(isnan(yvar1)) = interp1(xvar(~isnan(yvar1)), yvar1(~isnan(yvar1)), xvar(isnan(yvar1)) ); %just to join the plot line
%     figure; plot(xvar, yvar1 , '-r')
    % 2: scale within max range - DO NOT. WILL DO THE THIRD PART MANUALLY
    % IN ILLUSTRATOR 
    plot(xvar, yvar1, 'Color', colors(Ki,:).*0.3)
    
    % amplitude 2: dff 
%     yvar2 = [fTC(2).dffPeak(:,:,Ki), fTC(4).dffPeak(:,:,Ki)]; %dff values
    yvar2 = [fTC(2).dffPeak(:,:,Ki), NaN(1,8)]; %dff values
    yvar2 = yvar2(ia);
    yvar2(isnan(yvar2)) = interp1(xvar(~isnan(yvar2)), yvar2(~isnan(yvar2)), xvar(isnan(yvar2)) ); %just to join the plot line, in case of nan (none here)
%     figure; plot(xvar, yvar1 , '-r')
    % 2: scale within max range - DO NOT. WILL DO THE THIRD PART MANUALLY
    % IN ILLUSTRATOR 
    plot(xvar, yvar2, 'Color', colors(Ki,:).*0.65)
    
    % amplitude 3: dff 
    yvar3 = [fTC(3).dffPeak(:,:,Ki), NaN(1,8)]; %dff values
    yvar3 = yvar3(ia);
    yvar3(isnan(yvar3)) = interp1(xvar(~isnan(yvar3)), yvar3(~isnan(yvar3)), xvar(isnan(yvar3)) ); %just to join the plot line, in case of nan (none here)
%     figure; plot(xvar, yvar1 , '-r')
    % 2: scale within max range - DO NOT. WILL DO THE THIRD PART MANUALLY
    % IN ILLUSTRATOR 
    plot(xvar, yvar3, 'Color', colors(Ki,:))
    plot([xvar(1), xvar(end)],[0 0], '-k');   

    % amplitude 2 TONES: dff 
    xvar2 = chunks.tones.carrierFreqLevels;
    yvar2 = fTC(4).dffPeak(:,:,Ki); %dff values
    plot(xvar2, yvar2, 'x-','Color', colors(Ki,:).*0.65, 'MarkerSize',2)
    
    % amplitude 3 TONES: dff 
    xvar2 = chunks.tones.carrierFreqLevels;
    yvar2 = fTC(5).dffPeak(:,:,Ki); %dff values
    plot(xvar2, yvar2, 'x-','Color', colors(Ki,:), 'MarkerSize',2)
    
    
    hax(Ki).XLim = [xvar(1), xvar(end)];
    hax(Ki).YLim = dffLimits(Ki, :);
    hax(Ki).Box = 'off';
    hax(Ki).XAxis.Visible = 'off';
    hax(Ki).YTick = [];
...
for Ki = 1:nK
    hax(Ki).XAxis.Scale = 'log';
    hax(Ki).XAxis.MinorTick = 'off';
    hax(Ki).XMinorGrid = 'off';
end
    
% export and save!
hax(1).XAxis.FontSize = 4;
export_fig(fullfile(folder2figure2, 'resp_pips_ft_peaks_xlog.eps'))
...
cmap2 = interp1(x, cmap, y);
colormap(cmap2)
export_fig(fullfile(folder2figure2, 'pips_spectrogram.eps'))
colorbar off
ax.Box = 'on';
ax.XTick = [];
ax.YTick = [];
...
ax.XAxis.Color = [1,1,1];
ax.YAxis.Color = [1,1,1];
export_fig(fullfile(folder2figure2, 'pips_spectrogram.jpg'), '-m4')


%% map (anatomical position)
