...
function  GREENstack_registration_PP_f( imglistTarget, folder )
...
imglistGREEN = dir([folder '/smoothed_' imglistTarget]); %reference
...
%target
FileTif=fullfile(folder,imglistTarget);%'ImageStack.tif';
...
writetiff(stackUnSmoo, fullfile(folder,sprintf('registered_%s', imglistTarget)));
...
subplot(3,1,2);
distances = sqrt((shifts(:,3).^2) + (shifts(:,3).^2));
title('distance')
plot(distances)
subplot(3,1,3); 
...
name = [folder '/shifts_' imglistTarget];
% save
